...
class xmlNode(xmlCore):
    ...
    def addContent(self, content):
        """Append the extra substring to the node content. NOTE: In
           contrast to xmlNodeSetContent(), @content is supposed to
    ...
    def addContentLen(self, content, len):
        """Append the extra substring to the node content. NOTE: In
           contrast to xmlNodeSetContentLen(), @content is supposed
           to be raw text, so unescaped XML special chars are
           allowed, entity references are not supported. """
    ...
    def setContent(self, content):
        """Replace the content of a node. NOTE: @content is supposed
           to be a piece of XML CDATA, so it allows entity
           references, but XML special chars need to be escaped first
           by using xmlEncodeEntitiesReentrant() resp.
...
class SchemaValidCtxt(SchemaValidCtxtCore):
    ...
    def schemaValidateDoc(self, instance):
        if instance is None: instance__o = None
